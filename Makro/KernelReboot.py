...
class KernelReboot:
    """
    Reloads all modules in the Makro/ directory while preserving runtime session state.
    This includes logged-in user, terminal mode, and any other live context in flags.py.
    """

    def __init__(self):
        self.package = 'Makro'
        self.Makro_path = flags.base_folder
        self.exclude = {'MakroReloader', '__init__'}
        self.modules = self._find_modules()

    def _find_modules(self):
        """Discover reloadable modules inside Makro/."""
        return {
            name for _, name, _ in pkgutil.iter_modules([self.Makro_path])
            if name not in self.exclude
        }

    def reload_all(self):
        """Reload all Makro modules and restore important runtime state."""
        try:
            ...
            print(f"[MakroReloader] Reload failed: {e}")
            return False